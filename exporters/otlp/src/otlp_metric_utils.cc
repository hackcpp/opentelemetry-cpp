    ...
    proto::metrics::v1::Histogram *const histogram) noexcept
{
  ...
  for (auto &point_data_with_attributes : metric_data.point_data_attr_)
  {
    ...
    auto histogram_data = nostd::get<sdk::metrics::HistogramPointData>(point_data_with_attributes.point_data);
   // sum
...
{
  OtlpPopulateAttributeUtils::PopulateResource(resource_metrics->mutable_resource(),
                                                *(data.resource_));
  ...
  for (auto &scope_metrics : data.scope_metric_data_)
  {
    ...
    OtlpPopulateAttributeUtils::PopulateInstrumentationScope(scope, *scope_metrics.scope_);
