...
public:
  ...
  virtual bool isPresent(const std::string& key) const noexcept = 0;
...
public:
  ...
  bool isPresent(const std::string& /*key*/) const noexcept override { return true; }
};
...
public:
      ...
      const opentelemetry::common::KeyValueIterable &attributes) const noexcept override
  {
    ...
    attributes.ForEachKeyValue(
        [&](const std::string& key, opentelemetry::common::AttributeValue value) noexcept {
  ...
  bool isPresent(const std::string& key) const noexcept override
  {
