...
public:
  ...
  AttributeMap(
      std::initializer_list<std::pair<const std::string&, opentelemetry::common::AttributeValue>>
          attributes)
      : AttributeMap()
  {
...
public:
  ...
  // Construct map from initializer list by applying `SetAttribute` transform for every attribute
  OrderedAttributeMap(
      std::initializer_list<std::pair<std::string, opentelemetry::common::AttributeValue>>
          attributes)
      : OrderedAttributeMap()
  {
    for (auto &kv : attributes)
